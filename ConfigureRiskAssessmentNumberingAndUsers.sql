BEGIN TRANSACTION;
EXEC sp_rename N'[dbo].[RiskAssessment].[PreRiskAssessmentNumber]', N'RiskAssessmentNumber', 'COLUMN';

EXEC sp_rename N'[dbo].[PermitApplication].[PreRiskAssessmentNumber]', N'RiskAssessmentNumber', 'COLUMN';

ALTER TABLE [dbo].[RiskAssessment] ADD [PermitIssuerUserId] int NULL;

ALTER TABLE [dbo].[RiskAssessment] ADD [PermitReceiverUserId] int NULL;
...
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[RiskAssessment]') AND [c].[name] = N'PermitIssuerUserId');
IF @var IS NOT NULL EXEC(N'ALTER TABLE [dbo].[RiskAssessment] DROP CONSTRAINT ' + @var + ';');
ALTER TABLE [dbo].[RiskAssessment] ALTER COLUMN [PermitIssuerUserId] int NOT NULL;

DECLARE @var1 nvarchar(max);
SELECT @var1 = QUOTENAME([d].[name])
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[RiskAssessment]') AND [c].[name] = N'PermitReceiverUserId');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[RiskAssessment] DROP CONSTRAINT ' + @var1 + ';');
ALTER TABLE [dbo].[RiskAssessment] ALTER COLUMN [PermitReceiverUserId] int NOT NULL;

DECLARE @var2 nvarchar(max);
SELECT @var2 = QUOTENAME([d].[name])
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[RiskAssessment]') AND [c].[name] = N'PermitIssuerName');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[RiskAssessment] DROP CONSTRAINT ' + @var2 + ';');
ALTER TABLE [dbo].[RiskAssessment] DROP COLUMN [PermitIssuerName];

DECLARE @var3 nvarchar(max);
SELECT @var3 = QUOTENAME([d].[name])
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[dbo].[RiskAssessment]') AND [c].[name] = N'PermitReceiverName');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [dbo].[RiskAssessment] DROP CONSTRAINT ' + @var3 + ';');
ALTER TABLE [dbo].[RiskAssessment] DROP COLUMN [PermitReceiverName];

CREATE UNIQUE INDEX [IX_RiskAssessment_RiskAssessmentNumber] ON [dbo].[RiskAssessment] ([RiskAssessmentNumber]);

CREATE INDEX [IX_RiskAssessment_PermitIssuerUserId] ON [dbo].[RiskAssessment] ([PermitIssuerUserId]);

CREATE INDEX [IX_RiskAssessment_PermitReceiverUserId] ON [dbo].[RiskAssessment] ([PermitReceiverUserId]);

ALTER TABLE [dbo].[RiskAssessment] ADD CONSTRAINT [FK_RiskAssessment_Users_PermitIssuerUserId] FOREIGN KEY ([PermitIssuerUserId]) REFERENCES [dbo].[Users] ([Id]) ON DELETE NO ACTION;

ALTER TABLE [dbo].[RiskAssessment] ADD CONSTRAINT [FK_RiskAssessment_Users_PermitReceiverUserId] FOREIGN KEY ([PermitReceiverUserId]) REFERENCES [dbo].[Users] ([Id]) ON DELETE NO ACTION;

ALTER TABLE [dbo].[FiscalYearSetting] ADD CONSTRAINT [CK_FiscalYearSetting_NextNumbersNumeric] CHECK (LEN([NextRaNumber]) > 0 AND [NextRaNumber] NOT LIKE '%[^0-9]%' AND LEN([NextPaNumber]) > 0 AND [NextPaNumber] NOT LIKE '%[^0-9]%');
...
BEGIN
    ...
    JOIN [dbo].[ListItemCategory] category ON category.[ListItemCategoryId]=item.[ListItemCategoryId]
    WHERE category.[Code]=N'PERMIT_STATUS' AND item.[SystemName]=N'DRAFT';
...
BEGIN
    ...
    BEGIN TRY
        ...
        (
            ...
            WHERE permitApplication.[RiskAssessmentId]=@RiskAssessmentId AND permitStatus.[SystemName]<>N'DRAFT'
...
DECLARE @OldCamelName nvarchar(128)=LOWER(LEFT(N'PreRiskAssessmentNumber',1))+SUBSTRING(N'PreRiskAssessmentNumber',2,128);
DECLARE @NewCamelName nvarchar(128)=LOWER(LEFT(N'RiskAssessmentNumber',1))+SUBSTRING(N'RiskAssessmentNumber',2,128);
DECLARE procedure_cursor CURSOR LOCAL FAST_FORWARD FOR
    ...
    WHERE procedureObject.[type]=N'P' AND module.[definition] LIKE N'%PreRiskAssessmentNumber%';
OPEN procedure_cursor;
FETCH NEXT FROM procedure_cursor INTO @ProcedureDefinition;
WHILE @@FETCH_STATUS=0
BEGIN
    SET @ProcedureDefinition=REPLACE(@ProcedureDefinition,N'PreRiskAssessmentNumber',N'RiskAssessmentNumber');
    SET @ProcedureDefinition=REPLACE(@ProcedureDefinition,@OldCamelName,@NewCamelName);
    SET @ProcedureDefinition=REPLACE(@ProcedureDefinition,N'CREATE OR ALTER PROCEDURE',N'ALTER PROCEDURE');
    SET @ProcedureDefinition=REPLACE(@ProcedureDefinition,N'CREATE PROCEDURE',N'ALTER PROCEDURE');
...
VALUES (N'20260908185723_ConfigureRiskAssessmentNumberingAndUsers', N'10.0.9');
