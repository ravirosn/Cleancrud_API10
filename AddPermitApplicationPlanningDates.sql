BEGIN TRANSACTION;
ALTER TABLE [dbo].[PermitApplication] ADD [PlannedEndDateTime] datetime2(0) NULL;

ALTER TABLE [dbo].[PermitApplication] ADD [PlannedStartDateTime] datetime2(0) NULL;
...
BEGIN
    ...
    SET @Definition = REPLACE(@Definition, N'[RiskAssessmentNumber],[WorkLocation],[WorkDescription],[SpecialInstructions],', N'[RiskAssessmentNumber],[WorkLocation],[WorkDescription],[SpecialInstructions],[PlannedStartDateTime],[PlannedEndDateTime],');
    SET @Definition = REPLACE(@Definition, N'@LocationOfWork,COALESCE(@DescriptionOfWork,N''''),@SpecialInstructions,[ListItemId]', N'@LocationOfWork,COALESCE(@DescriptionOfWork,N''''),@SpecialInstructions,@PlannedStartDateTime,@PlannedEndDateTime,[ListItemId]');
    SET @Definition = REPLACE(@Definition, N'[WorkDescription]=COALESCE(@DescriptionOfWork,N''''),[SpecialInstructions]=@SpecialInstructions,', N'[WorkDescription]=COALESCE(@DescriptionOfWork,N''''),[SpecialInstructions]=@SpecialInstructions,[PlannedStartDateTime]=@PlannedStartDateTime,[PlannedEndDateTime]=@PlannedEndDateTime,');
...
VALUES (N'20260910192326_AddPermitApplicationPlanningDates', N'10.0.9');
